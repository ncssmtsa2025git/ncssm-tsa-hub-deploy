...
export async function getTeamCheckins(teamId: string): Promise<Checkin[]> {
  const res = await fetch(`${API_BASE}/teams/${teamId}/checkins`, {
    method: "GET",
    credentials: "include",
  });
...
export async function createCheckin(teamId: string, payload: CheckinCreate): Promise<Checkin> {
  const res = await fetch(`${API_BASE}/teams/${teamId}/checkins`, {
    method: "POST",
    credentials: "include",
